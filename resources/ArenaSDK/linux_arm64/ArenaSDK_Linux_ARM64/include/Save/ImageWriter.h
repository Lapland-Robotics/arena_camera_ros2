...
namespace Save
{
	...
	public:
		...
		/**
		 * @fn virtual void SetTiff(const char* pSetExtension = ".tiff", ETiffCompression compression = NoCompression, bool cmykTags = false)
		 *
		 * @param pSetExtension
		 *  - Type: const char*
		 *  - Default: ".tiff"
		...
		*/
		...
		/**
		 * @fn static void SaveRawData(const char* pFileName, uint8_t* pImageData, const size_t size)
		 *
		 * @param pFileName
		 *  - Type: const char*
		 ...
		 * @param size
		 *  - Type: const size_t
		 *  - Size of the file to save
		 *
		 * <B> SaveRawData </B> is a static function that saves the raw data of a
		 * buffer.
		 *
		 */
		static void SaveRawData(const char* pFileName, uint8_t* pImageData, const size_t size);

#ifdef _WIN32

		static void SaveRawData(const wchar_t* pFileName, uint8_t* pImageData, const size_t size);
