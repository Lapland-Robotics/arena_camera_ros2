...
namespace Arena
{
	...
	public:
		...
		/**
		 * @fn virtual uint64_t GetLeaderReceivedTimestamp()
		 *
		 * @return 
		 *  - Type: uint64_t
		 ...
		 *
		 * <B> GetLeaderReceivedTimestamp </B> gets the timestamp of when the leader
		 * was received. Images are self-describing, so the device does not need to
		 * be queried to get this information.
		 *
		 * Compressed images are retrieved from a device (Arena::IDevice). If the 
		 * image was retrieved from a device, the timestamp is populated by the
		 * acquisition engine on receiving the leader. The device itself is not queried
		 * as this data is present in the image data.
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued.
		 ...
		 *  - Arena::IDevice
		 */
		virtual uint64_t GetLeaderReceivedTimestamp() = 0;
