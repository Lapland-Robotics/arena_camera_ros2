...
namespace Arena
{
	...
	public:
		...
		/**
		 * @fn static IImage* Overlay(IImage* pDepthImage, IImage* pRgbImage, OverlayInfo* pOverlayInfo);
		 *
		 * @param pImage
		 *  - Type: Arena::IImage
		 *  - Pointer to the depth image to overlay
		 *
		 * @param pImage
		 *  - Type: Arena::IImage
		 *  - Pointer to the RGB image to overlay
		 * 
		 * @param pImage
		 *  - Type: Arena::OverlayInfo
		 *  - Pointer to the overlay information obtained from the helios-chroma device group
		 * 
		 * @return 
		 *  - Type: IImage*
		 *  - Pointer to the image with the data of RGB values of each 3D point
		 *
		 * <B> Overlay </B> takes depth image, RGB image and overlay information
		 *  and overlays the RGB data onto the depth image.
		 *
		 * @see 
		 *  - Arena::IImage
		 */
		static IImage* Overlay(IImage* pDepthImage, IImage* pRgbImage, OverlayInfo* pOverlayInfo);
		static OverlayInfo* CreateOverlayInfo(IDevice* pChroma, IDevice* pHelios);
		static void DestroyOverlayInfo(OverlayInfo* pOverlayInfo);
