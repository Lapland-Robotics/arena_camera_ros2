...
namespace Arena
{
	...
	public:
		 ...
		 *
		 * <B> GetLeaderReceivedTimestamp </B> gets the timestamp of when the leader was received. 
		 * Images are self-describing, so the device does not need to be queried to get this
		 * information.
		 *
		 * Images are either retrieved from a device (Arena::IDevice) or created
		 * by the image factory (Arena::ImageFactory). If the image was retrieved
		 * from a device, the leader-received timestamp is populated by the acquisition engine
		 * on receiving the leader. The device itself is not queried as this data is
