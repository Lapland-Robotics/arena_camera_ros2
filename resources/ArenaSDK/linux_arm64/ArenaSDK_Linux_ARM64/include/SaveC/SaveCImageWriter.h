...
/**
 * @fn SC_ERROR SC_API saveWriterCreate(size_t width, size_t height, size_t bitsPerPixel, size_t numVertices, saveWriter* phWriter)
 *
 * @param width
 *  - Type: size_t
 *  - [In] parameter
 ...
 *  - Type: size_t
 *  - [In] parameter
 *  - Number of valid vertices in the image
 *  - This is applicable to XYTP ply images
 * 
 * @return 
 *  - Type: SC_ERROR
 *  - Error code for the function
 *  - Returns SC_ERR_SUCCESS (0) on success
 *
 * <B> saveWriterCreateXYTP </B> creates an image writer while setting the parameters
 * of the upcoming image(s) to save. The default file name pattern is
 * 'savedimages/image<count>.jpg' and the default file format is jpeg.
 */
SC_ERROR SC_API saveWriterCreateXYTP(size_t width, size_t height, size_t bitsPerPixel, size_t numVertices, saveWriter* phWriter);
...
/**
 * @fn SC_ERROR SC_API saveWriterSaveRawData(const char* pFileName, uint8_t* pImageData, const size_t size)
 *
 * @param pFileName
 *  - Type: const char*
 *  - [In] parameter
 ...
 *  - Returns SC_ERR_SUCCESS (0) on success
 *
 * <B> saveReaderLoadRawData </B> save data buffer into a raw file. It does not
 * require an image writer context (saveWriter).
 *
 * @warning 
 ...
 *  - saveWriter
 */
SC_ERROR SC_API saveWriterSaveRawData(const char* pFileName, uint8_t* pImageData, const size_t size);
#ifdef _WIN32
SC_ERROR SC_API saveWriterSaveRawDataU(const wchar_t* pFileName, uint8_t* pImageData, const size_t size);
#endif
