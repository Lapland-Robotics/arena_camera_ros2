...
/**
 * @fn AC_ERROR AC_API acCompressedImageGetLeaderReceivedTimestamp(acBuffer hBuffer, uint64_t* pLeaderReceivedTimestamp)
 *
 * @param hBuffer
 *  - Type: acBuffer
 *  - [In] parameter
 *  - An image
 *
 * @param pLeaderReceivedTimestamp
 *  - Type: uint64_t*
 *  - Unit: count of 100-nanosecond intervals on Windows 
 *          count of nanoseconds on Linux
 *  - [Out] parameter
 *  - Windows: The leader receive time as a count of 100-nanosecond intervals since 
 *    January 1, 1601
 *  - Linux: The leader receive time as the number of nanoseconds since January 1, 1970
 * 
 * @return 
 *  - Type: AC_ERROR
 *  - Error code for the function
 *  - Returns AC_ERR_SUCCESS (0) on success
 *
 * <B> acCompressedImageGetLeaderReceivedTimestamp </B> gets the timestamp of when the leader was received.
 * Compressed images are self-describing, so the device does not need to be queried to get
 * this information.
 */
AC_ERROR AC_API acCompressedImageGetLeaderReceivedTimestamp(acBuffer hBuffer, uint64_t* pLeaderReceivedTimestamp);
