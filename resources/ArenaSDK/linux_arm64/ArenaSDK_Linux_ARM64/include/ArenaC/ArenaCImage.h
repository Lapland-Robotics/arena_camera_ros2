...
/**
 * @fn AC_ERROR AC_API acImageGetLeaderReceivedTimestamp(acBuffer hBuffer, uint64_t* pLeaderReceivedTimestamp)
 *
 * @param hBuffer
 *  - Type: acBuffer
 *  - [In] parameter
 ...
 *  - Returns AC_ERR_SUCCESS (0) on success
 *
 * <B> acImageGetLeaderReceivedTimestamp </B> gets the timestamp of when the leader was received.
 * Images are self-describing, so the device does not need to be queried to get
 * this information.
 */
AC_ERROR AC_API acImageGetLeaderReceivedTimestamp(acBuffer hBuffer, uint64_t* pLeaderReceivedTimestamp);
