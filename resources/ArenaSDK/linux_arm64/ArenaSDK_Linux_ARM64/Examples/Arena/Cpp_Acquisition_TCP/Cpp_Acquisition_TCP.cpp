...
void AcquireImages(Arena::IDevice* pDevice)
{
	...
	if (GenApi::IsImplemented(pDevice->GetNodeMap()->GetNode("TransportStreamProtocol")))
	{
		...
		// The DeviceStreamProtocol node informs the Arena SDK to use the non-standard "TransportStreamProtocol" node
		//  to control the stream type when it is set to "Legacy".  This node can also be set to "Automatic" and in that
		//  case Arena will decide to use either GVSP or GVRSP based on these factors:
		//  GVRSP will be used when all these are true:
		//		1) Host hardware supports RDMA
		//		2) Device supports GigeVision 3.0 (GVRSP)
		//		3) Device supports GenDC
		//  othersise the device will fall back to using GVSP.
		// This node can also be used to force GVSP or GVRSP by settingit to "Gvsp" or "Gvrsp".  If a user forces GVRSP but
		// but the hardware does not support it, the IDevice::StartStream() call will throw an error.
		Arena::SetNodeValue<GenICam::gcstring>(
			pDevice->GetTLDeviceNodeMap(),
			"DeviceStreamProtocol",
			"Legacy");
