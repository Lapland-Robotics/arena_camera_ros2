...
//    saved as a PLY file.
#define FILE_NAME "Images/C_EVS_Save_Ply/C_EVS_Save_Ply.ply"
...
AC_ERROR SaveImage(acBuffer hBuffer, const char* filename)
{
	...
	saveErr = saveWriterCreateXYTP(width, height, bpp, numVertices, &hWriter);
	...
	acErr = saveWriterGetLastFileName(hWriter, fileName, &fileLen);
	if (acErr != AC_ERR_SUCCESS)
		return acErr;
...
// (9) stops the stream
AC_ERROR AcquireAndSaveImage(acDevice hDevice)
{
	AC_ERROR err = AC_ERR_SUCCESS;
...
int main()
{
	...
	err = AcquireAndSaveImage(hDevice);
	CHECK_RETURN;
	printf("\nExample complete\n");
